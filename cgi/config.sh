...
if [[ `echo "show tables;" | $DB_COMMAND | wc -l` -eq 0 ]]; then
	...
	echo "create table if not exists posts (id int auto_increment not null primary key, login varchar(32), title varchar(32) not null, content varchar(6000) not null, time datetime default now() not null);" | $DB_COMMAND > /dev/null
	echo "create table if not exists comments (id int auto_increment not null primary key, postid int not null, login varchar(32), content varchar(100) not null, time datetime default now() not null);" | $DB_COMMAND > /dev/null
fi
