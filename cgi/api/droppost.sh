...
if [[ "$QUERY_STRING" != "" ]]; then
	SQL="select COUNT(*) from posts where login='$QUERY_LOGIN' and id='$QUERY_STRING'"
	RECORDS=`echo "$SQL" | $DB_COMMAND | tail -n +2`
	if [[ "$RECORDS" -gt 0 ]] || [[ "$USER_ADMIN" == true ]]; then
		SQL="delete from posts where id='$QUERY_STRING'"
		RECORDS=`echo "$SQL" | $DB_COMMAND 2>&1 | wc -l`
		if [[ "$RECORDS" -eq 0 ]]; then
			SQL="delete from comments where postid='$QUERY_STRING'"
			RECORDS=`echo "$SQL" | $DB_COMMAND 2>&1 | wc -l`
			if [[ "$RECORDS" -eq 0 ]]; then
